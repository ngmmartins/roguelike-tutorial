use crate::{
    particle_system::ParticleBuilder, CombatStats, Equippable, Equipped, MagicMapper, RunState,
    WantsToRemoveItem,
};

use super::{
    gamelog::GameLog, AreaOfEffect, Confusion, Consumable, HungerClock, HungerState, InBackpack,
    InflictsDamage, Map, Name, Position, ProvidesFood, ProvidesHealing, SufferDamage,
        ...
        for (entity, useitem) in (&entities, &wants_use).join() {
            ...
            match item_edible {
                ...
                Some(_) => {
                    used_item = true;
                    let target = targets[0];
                    let hc = hunger_clocls.get_mut(target);
                    if let Some(hc) = hc {
